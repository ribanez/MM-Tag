...
def readfile(datafile, range_len=None, is_lower=False):
    """
    Helper to read data files, the files contain:
    * first item on each line is a word
    * second a part-of-speech (POS) tag
    * third a syntactic chunk tag
    * fourth the named entity tag
    :param datafile: (string) path to data file
    :param range_len: (array) [min_length, max_length] length's constain, consider only documents with then
    quantity of words
    :param is_lower: (boolean) indicate if the words must be lower
    :return: [word_list, pos_list, chunk_list, ner_list]
            word_list: (array) with arrays of words.
            pos_list: (array) with arrays of pos.
            chunk_list: (array) with arrays of chunks.
            ner_list: (array) with arrays of ners.
    """
    ...
    if range_len is not None:
        word_list = [l for l in word_list if len(l) < range_len[1] and len(l) > range_len[0]]
        pos_list = [l for l in pos_list if len(l) < range_len[1] and len(l) > range_len[0]]
        chunk_list = [l for l in chunk_list if len(l) < range_len[1] and len(l) > range_len[0]]
        ner_list = [l for l in ner_list if len(l) < range_len[1] and len(l) > range_len[0]]
...
def merge_vocab(train, valid, test):
    """
    Helper to merge vocabularies
    :param train: (array) of arrays the items in train_set
    :param valid: (array) of arrays the items in valid_set
    :param test: (array) of arrays the items in test_set
    :return: (dict) item with a index
    """
    vocab = {}
...
def build_train_vocab(x_train):
    """
    Helper to build train vocabulary
    :param x_train: (array) of arrays the items in train_set
    :return: (dict) with frequencies of words in dataset
    """
    x_vocab = {}
    for line in x_train:
        for w in line:
...
def build_char_vocab(x_train):
    """
    Helper to build train char vocabulary
    :param x_train: (array) of arrays the items in train_set
    :return: (dict) with characters and an index
    """
    char_list = []
    for line in x_train:
        for w in line:
...
def load_pretrain_embedding(pretrain_embed_file):
    """
    Helper to load pre-trained embedding from a file
    :param pretrain_embed_file: (string) with path to de pre-trained embedding path
    :return: (dict) with words and their respective embedding
    """
...
def add_unknown_words(word_vecs, vocab, min_df=100, k=100):
    ...
    :param word_vecs: (dict) with words and their respective embedding
    :param vocab: (dict) with words and their frequency
    :param min_df: (int) number of minimum occurencies
    :param k: (int) embedding size
    :return: (dict) with words and their respective embedding (random vector if not exist pre-trained)
    """
    unk_count = 0
    for word in vocab:
...
def get_w(word_vecs, k=100):
    """
    helper to get word embedding and word index dictionary. word_embed[i] is the vector for word indexed by i.
    :param word_vecs: (dict) with words and their respective embedding
    :param k: (int) embedding size
    :return: [word_embed, word_idx_map]
        * word_embed: (numpy array) all word embeddings in a matrix
        * word_idx_map: (dict) with word and its index in the matrix.
    """
...
def check_embed_file(fname1, fname2):
    """
    helper to check if two embedding files have the same word id
    :param fname1: (string) path to file 1
    :param fname2: (string) path to file 2
    :return: (boolean) True if have the same word id, false in other case.
    """
...
def word2index(docs, vocab, max_len, need_pad=False):
    """
    helper to map documents in index
    :param docs: (array) with the documents (arrays of words)
    :param vocab: (dict) with words and their indexes
    :param max_len: (int) max length
    :param need_pad: (boolean) indicate if the arrays need add padding
    :return: (numpy array) with indexes representation of documents
    """
    index_docs = []
    for doc in docs:
        index_d = []
...
def load_pos_chunk_ner_label():
    """
    helper to load vocabularies previosly preprocessed
    :return: [pos_vocab, chunk_vocab, ner_vocab]
        * pos_vocab: (dict) with pos and their indexes
        * chunk_vocab: (dict) with chuck and their indexes
        * ner_vocab: (dict) with ner and their indexes
    """
...
def pos2index(pos, pos_vocab, max_len, need_pad=False):
    """
    helper to map pos in index
    :param pos: (array) with the documents (arrays of pos)
    :param pos_vocab: (dict) with pos and their indexes
    :param max_len: (int) max length
    :param need_pad: (boolean) indicate if the arrays need add padding
    :return: (numpy array) with indexes representation of documents
    """
...
def chunk2index(chunk, chunk_vocab, max_len, need_pad=False):
    """
    helper to map chunk in index
    :param chunk: (array) with the documents (arrays of chunk)
    :param chunk_vocab: (dict) with chuck and their indexes
    :param max_len: (int) max length
    :param need_pad: (boolean) indicate if the arrays need add padding
    :return: (numpy array) with indexes representation of documents
    """
...
def ner2index(ner, ner_vocab, max_len, need_pad=False):
    """
    helper to map chunk in index
    :param ner: (array) with the documents (arrays of ner)
    :param ner_vocab: (dict) with ner and their indexes
    :param max_len: (int) max length
    :param need_pad: (boolean) indicate if the arrays need add padding
    :return: (numpy array) with indexes representation of documents
    """
...
def char2index(data, char_vocab):
    """
    helper to map documents in char index
    :param data: (array) with documents (array of words)
    :param char_vocab: (dict) with chars and their indexes
    :return: (numpy array) with char indexes representation of documents
    """
    char_ids = []
    for line in data:
        chars_per_line = []
...
def bio_iob(tags):
    """
    helper to convert BIO to IOB
    :param tags: (array) with tags in BIO representation
    :return: (array) with tags in IOB representation
    """
    new_tags = []
...
def iob_bio(tags):
    """
    helper to convert IOB to BIO
    :param tags: (array) with tags in IOB representation
    :return: (array) with tags in BIO representation
    """
    new_tags = []
    for i, tag in enumerate(tags):
        if 'I' in tag:
...
def bio_bioes(tags):
    """
    helper to convert BIO to BIOES
    :param tags: (array) with tags in BIO representation
    :return: (array) with tags in BIOES representation
    """
    new_tags = []
    length = len(tags)
    for i, tag in enumerate(tags):
...
def bioes_bio(tags):
    """
    helper to convert BIOES to BIO
    :param tags: (array) with tags in BIOES representation
    :return: (array) with tags in BIO representation
    """
    new_tags = []
    length = len(tags)
